...
from parsy import alt, string, regex, seq
...
def create_description_parser():
    ...
    QUALIFIED_NAME = regex(r'[^\.;\[/\(\):]+').sep_by(string('/')).combine(joining('.'))
    PRIMITIVE_TYPE = regex('[BCDFIJSZ]').map( lambda i: JVM_PRIMITIVE_TYPES[i])
    CLASS_NAME = string('L') >> QUALIFIED_NAME << string(';')
    ARRAY = seq(string('[').at_least(1).map( lambda o: '[]' * len(o)), (PRIMITIVE_TYPE | CLASS_NAME )).combine(lambda o, t: t + o)
    TYPE = PRIMITIVE_TYPE | CLASS_NAME | ARRAY
    RETURN_TYPE = string('V').map(lambda _: 'void') | TYPE
    DESCRIPTION = seq(string('(') >> TYPE.many().combine(joining(',')), string(')') >> RETURN_TYPE).combine(lambda params, rt: ('(' + params + ')', rt))
...
TEST_CASE_PATTERN = re.compile(r"^(?P<class>.+)\.(?P<method>[^\[]+)((?P<params>\[.*\])?\((?P=class)?\))?$")
...
def get_hints(hint_folder, test_cases, method_locations):
    ...
    for item in hint_data:
        ...
        if 'accessors' in item:
            hint['targets'] = [ method_name(entry) for entry in item['accessors']]
    ...
    for mutation in data['mutations']:
        ...
        for case in tests:
            ...
            if not match:
                ...
                new_tests.append({'class': match.group("class"), 'method':match.group("method")})
        result[mutation_id(mutator=mutation['mutator'], method=mutation['method']['name'], **mutation['method'])] = new_tests
    return result
...
def create_template(project_info):
    ...
    def file_url(path):
        base_folder = join(project_info['project'].split('/')[-1], project_info['folder'])
...
def generate_report_for_project(folder):
    ...
    method_locations = load_method_locations(xml_report_path) if exists(xml_report_path) else {}  # Too bad I don't have the line number in the JSON report :(
    all_hints = glob(join(folder, 'observations', 'methods', '*', '*', 'hints.json')) + glob(join(folder, 'observations', 'tests', '*', 'hints.json'))
    print(join(folder, 'observations', 'methods', '*', '*', 'hints.json'))
    for hint_file in all_hints:
        print(hint_file)
        generate_readable_report(dirname(hint_file), template, test_cases, method_locations)
