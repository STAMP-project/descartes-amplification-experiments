...
public final class JCodeModelTest {
    ...
    @Test(timeout = 10000)
    public void testEmptyNarrowed_mg81null2434_failAssert0() throws JClassAlreadyExistsException {
        try {
            String __DSPOT_sField_36 = null;
            final JCodeModel cm = new JCodeModel();
            final JDefinedClass jClass = cm._class("EmptyNarrowed", EClassType.INTERFACE);
            final AbstractJClass hashMap = cm.ref(HashMap.class).narrowEmpty();
            JFieldVar o_testEmptyNarrowed_mg81__9 = jClass.field(JMod.PRIVATE, cm.ref(Map.class).narrow(String.class), "strMap", JExpr._new(hashMap));
            CodeModelTestsHelper.parseCodeModel(cm);
            hashMap.staticRef(__DSPOT_sField_36);
            org.junit.Assert.fail("testEmptyNarrowed_mg81null2434 should have thrown IllegalArgumentException");
        } catch (IllegalArgumentException expected) {
            Assert.assertEquals("The expression must be false but it is not: name or var must be present", expected.getMessage());
        }
    }
}
