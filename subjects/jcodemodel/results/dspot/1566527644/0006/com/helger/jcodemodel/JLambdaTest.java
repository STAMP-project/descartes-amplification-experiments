...
public final class JLambdaTest {
    ...
    @Test(timeout = 10000)
    public void testIssue62_mg106_failAssert0() throws Exception {
        try {
            int __DSPOT_index_40 = 113152347;
            ...
            String String_106 = (((((((((("arr -> {" + (JLambdaTest.CRLF)) + "    synchronized (a)") + (JLambdaTest.CRLF)) + "    {") + (JLambdaTest.CRLF)) + "        a.entrySet().retainAll(arr.entrySet());") + (JLambdaTest.CRLF)) + "    }") + (JLambdaTest.CRLF)) + "}") + (JLambdaTest.CRLF);
            CodeModelTestsHelper.toString(aLambda);
            setBody.remove(__DSPOT_index_40);
            org.junit.Assert.fail("testIssue62_mg106 should have thrown IndexOutOfBoundsException");
        } catch (IndexOutOfBoundsException expected) {
            Assert.assertEquals("Index: 113152347, Size: 1", expected.getMessage());
        }
    }

    @Test(timeout = 10000)
    public void testIssue62_literalMutationString24_failAssert0() throws Exception {
        try {
            final JCodeModel cm = new JCodeModel();
            final JVar holder = new JVar(JMods.forVar(0), cm.ref(Object.class), "a", null);
            final JLambda aLambda = new JLambda();
            final JLambdaParam arr = aLambda.addParam("a,rr");
            final JBlock setBody = aLambda.body().synchronizedBlock(holder).body();
            setBody.add(JExpr.invoke(cm, holder, "entrySet").invoke("retainAll").arg(arr.invoke("entrySet")));
            String String_24 = (((((((((("arr -> {" + (JLambdaTest.CRLF)) + "    synchronized (a)") + (JLambdaTest.CRLF)) + "    {") + (JLambdaTest.CRLF)) + "        a.entrySet().retainAll(arr.entrySet());") + (JLambdaTest.CRLF)) + "    }") + (JLambdaTest.CRLF)) + "}") + (JLambdaTest.CRLF);
            CodeModelTestsHelper.toString(aLambda);
            org.junit.Assert.fail("testIssue62_literalMutationString24 should have thrown IllegalArgumentException");
        } catch (IllegalArgumentException expected) {
            Assert.assertEquals("The expression must be true but it is not: Illegal variable name \'a,rr\'", expected.getMessage());
        }
    }
