...
public class BetweenDayOfWeekValueGeneratorTest {
    @Test(timeout = 10000)
    public void testGenerateCandidatesNotIncludingIntervalExtremes_literalMutationNumber16_failAssert0() throws Exception {
        try {
            ...
            BetweenDayOfWeekValueGenerator generator = new BetweenDayOfWeekValueGenerator(cronField, 2015, 2, new WeekDay(1, true));
            List<Integer> values = generator.generateCandidates(1, 29);
            values.contains(29);
            org.junit.Assert.fail("testGenerateCandidatesNotIncludingIntervalExtremes_literalMutationNumber16 should have thrown DateTimeException");
        } catch (DateTimeException expected) {
            Assert.assertEquals("Invalid date \'February 29\' as \'2015\' is not a leap year", expected.getMessage());
        }
    }
