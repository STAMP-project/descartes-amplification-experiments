...
public class HtmlParserTest {
    ...
    @Test(timeout = 10000)
    public void tracksLimitedErrorsWhenRequested_add9_literalMutationString4119_failAssert0() throws Exception {
        try {
            String html = "}5^f3RUL01$Q{+L]Hc?<k}3C5#]hXBx>O&<9:HO)kSSzS.?^!e [UCxI>Qf0";
            Parser parser = Parser.htmlParser().setTrackErrors(3);
            Document doc = parser.parseInput(html, "http://example.com");
            List<ParseError> errors = parser.getErrors();
            int o_tracksLimitedErrorsWhenRequested_add9__9 = errors.size();
            errors.get(0).toString();
            ParseError o_tracksLimitedErrorsWhenRequested_add9__12 = errors.get(1);
            errors.get(1).toString();
            errors.get(2).toString();
            org.junit.Assert.fail("tracksLimitedErrorsWhenRequested_add9_literalMutationString4119 should have thrown IndexOutOfBoundsException");
